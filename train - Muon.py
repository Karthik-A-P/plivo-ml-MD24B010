...
def get_lr(step, args, warmup=100, min_lr_ratio=0.1):
    if step < warmup:
        return args.lr * step / warmup
    if step >= args.steps:
        return args.lr * min_lr_ratio
    ratio = (step - warmup) / max(1, args.steps - warmup)
    coeff = 0.5 * (1 + math.cos(math.pi * ratio))
    return args.lr * min_lr_ratio + coeff * args.lr * (1 - min_lr_ratio)
...
def main():
    ...
    for name, p in model.named_parameters():
        if p.ndim == 2 and "emb" not in name:      # 2D hidden weights -> Muon
            muon_params.append(p)
        else:                                       # embeddings/norms/biases -> AdamW
            adamw_params.append(p)

    opt_muon  = Muon(muon_params, lr=0.02, momentum=0.95)
    opt_adamw = torch.optim.AdamW(adamw_params, lr=args.lr, betas=(0.9, 0.95),
                                  weight_decay=0.1)
    ...
    for step in range(1, args.steps + 1):
        ...
        for o in optimizers:
            for g in o.param_groups:
                g["lr"] = get_lr(step, args) if o is opt_adamw else 0.02 * (get_lr(step, args) / args.lr)
        _, loss = model(x, y)
        for o in optimizers:
